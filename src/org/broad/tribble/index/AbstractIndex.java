...
public abstract class AbstractIndex implements Index {
    ...
    protected void printIndexInfo() {
        log.info(String.format("Index for %s with %d indices", indexedFile, chrIndices.size()));
        BlockStats stats = getBlockStats(true);
        log.info(String.format("  total blocks %d", stats.total));
        log.info(String.format("  total empty blocks %d", stats.empty));
    }
    ...
    protected BlockStats getBlockStats(boolean logDetails) {
        ...
        for (Map.Entry<String, ChrIndex> elt : chrIndices.entrySet()) {
            ...
            if (blocks != null) {
                ...
                if (logDetails)
                    log.info(String.format("  %s => %d blocks, %d empty, %.2f", elt.getKey(), nBlocks, nEmptyBlocks, (100.0 * nEmptyBlocks) / nBlocks));
            }
        }
