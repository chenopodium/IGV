...
public class FeatureTreeNode {
    ...
    protected FeatureTrack source;
    private boolean compact;
    private int MAX_FEATURES_BEFORE_COMPACT = 100;
    // filter
    private int nrFilterPassed;

    public FeatureTreeNode(FeatureTrack source, String chr, int start, int end, int nrbuckets) {
    ...
    public List<KaryoFeature> getFilteredFeatures(KaryoFilter filter, boolean debug) {
        if (!filter.isValid()) {
            if (debug) p("getFilteredFeatures(filter): Filter "+filter+" is not valied, returning null");
            return null;
        }
        if (debug) p("getFilteredFeatures(filter): Filter "+filter+" is valid");
        ...
        if (debug) p("getFilteredFeatures(filter): Filter "+filter+" is valid, got "+res.size()+" filtered features for node "+this);
        return res;
    }
    ...
    private void loadFeatures() {
        ...
        List<Feature> res = source.getFeatures(chr, getStart(), getEnd());
        features = new ArrayList<KaryoFeature>();

        if (res != null) {
            if (source instanceof FakeCnvTrack) {
                p("Loading features of FakeCnvTrack for node from " + getStart() + "-" + getEnd() + ", and got:" + res.size() + " results");
            }
            for (Feature f : res) {
