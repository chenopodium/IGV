...
public class WizardContainer extends JPanel implements WizardController {
   ...
   private void initComponents() {
      ...
      _navPanel = new JPanel();
      WizardPage[] pages = _factory.getAllPages();
      _navPanel.setLayout(new GridLayout(pages.length, 1));
      int step = 0;
      for (WizardPage page: pages) {
          step++;
          JLabel lbl = new JLabel(step+") "+page.getTitle());
          _navPanel.add(lbl);
      }
      
      this.add(_template, BorderLayout.CENTER);
      this.add(buttonPanel, BorderLayout.SOUTH);
      this.add(new FlowPanel(_navPanel), BorderLayout.WEST);
   }
