...
public class GuiCNVTree extends GuiFeatureTree {

// ***************************************************************************
// FROM DRAWABLE
// ***************************************************************************
    public GuiCNVTree(KaryoTrack ktrack, DrawingCanvas canvas, GuiChromosome chromo, FeatureTree tree, int dx) {
        this(ktrack, canvas, chromo, tree, dx, 1.0);
    }

    public GuiCNVTree(KaryoTrack ktrack, DrawingCanvas canvas, GuiChromosome chromo, FeatureTree tree, int dx, double scale) {
...
//        nrpixelperfeature = nrpixelperfeature/2;
        ...
        Color cpos = ktrack.getDefaultColor();
        Color sneg = this.getInvertedColor(cpos);
        drawVars(cpos, false, drawall, gains, filter, g, nrpixelperfeature, s, e, x - w / 2, startx, y, h, w / 2, node, maxnr);

        drawVars(sneg, true, drawall, losses, filter, g, nrpixelperfeature, s, e, x - w / 2, startx, y, h, w / 2, node, maxnr);
    }

    protected void drawVars(Color basecolor, boolean toLeft, boolean drawall, ArrayList<KaryoFeature> vars, KaryoFilter filter, Graphics g, double nrpixelperfeature, int s, int e, int x, double startx, int y, int h, int w, FeatureTreeNode node, int maxnr) {
        int nr = vars.size();
        ...
        if (drawall) {
            for (int i = 0; i < nr; i++) {
                ...
                g.setColor(getColor(basecolor, copynr));
                boolean drawit = true;
                ...
                if (drawit) {
                    ...
                    if (toLeft) {
                        ...
                        g.fillRect(x + (int) startx, y1 + y - h, wb, y2 - y1);
                        g.drawRect(x + (int) startx, y1 + y - h, wb, y2 - y1);
                    }
    ...
    private Color getColor(Color color, int copynr) {
        int MAX = 50;
        copynr = Math.abs(Math.min(MAX, copynr));
        double dr = color.getRed() / (MAX*2);
        double dg = color.getGreen() / (MAX*2);
        double db = color.getBlue() / (MAX*2);
