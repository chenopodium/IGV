...
package com.iontorrent.sam2flowgram.flowalign;
...
public class IonogramAlignment {
    ...
    /**
     * The alignment was extended from a skipped match (skipping).
     */
    public static final int FROM_SKIPPEDMATCH = 5;
    /**
     * The alignment was extended from a skipped insertion (skipping).
     */
    public static final int FROM_SKIPPEDINS = 6;
    /**
     * The alignment was extended from an insertion.
     */
     ...
     */
    public IonogramAlignment(FlowSeq flowQseq, byte tseq[],
     ...
     */
    public IonogramAlignment(FlowSeq flowQseq, byte tseq[], FlowOrder qseqFlowOrder,
    ...
    public IonogramAlignment(FlowSeq flowQseq, String consensus, FlowOrder qorder, boolean startLocal, boolean endLocal, int gapPenalty)
     throws Exception {
    ...
    public FlowValue getQueryFlow(int queryflowpos) {
        return this.flowQseq.getFlow(queryflowpos);
    }
    public FlowValue getTargetFlow(int targetflowpos) {
    ...
    public void computeAlignment(FlowSeq flowQseq, byte targetseq[], FlowOrder qseqFlowOrder,
        ...
        for (i = 0; i <= flowQseq.getLength(); i++) {
            ...
            if (i > 0) {
                ...
                if (0 == iFrom) {
                    ...
                    dp[i][0].insFrom = FROM_SKIPPEDINS;
                } else {
            ...
            for (j = 1; j <= flowTseq.getLength(); j++) { // target
                ...
                if (j == flowTseq.getLength() // no next reference base
                    ...
                    if (dp[i - 1][j].insScore <= dp[i - 1][j].matchScore) {
                        vScoreE = dp[i - 1][j].matchScore - qscore;
                        vFromE = FROM_MATCHEMPTY;
                    } else {
                        vScoreE = dp[i - 1][j].insScore - qscore;
                        vFromE = FROM_INSEMPTY;
                    }
                    // Start anywhere in tseq
                    if (i == 1 && vScoreE + qscore < 0) {
                        vScoreE = 0 - qscore;
                        vFromE = FROM_S;
                    }
                }
                ...
                if (dp[iFrom][j].insScore <= dp[iFrom][j].matchScore) {
                    ...
                    vFromP = FROM_SKIPPEDMATCH;
                } else {
                    vScoreP = dp[iFrom][j].insScore - gapSumsI[i - 1];
                    vFromP = FROM_SKIPPEDINS;
                }
                // compare empty vs. skipped
                if (vScoreP <= vScoreE) { // Note: always choose empty over skipped
                ...
                if (qseqFlowOrder.flowOrder[(i - 1) % qseqFlowOrder.getLength()] != tseqFlowOrder.flowOrder[(j - 1) % tseqFlowOrder.getLength()]) {
                    ...
                    double signaldiff = ((qscore < tscore) ? (tscore - qscore) : (qscore - tscore));
                    // NB: do not penalize it full on the first or last flow
                    if (i == 1 || i == flowQseq.getLength()) {
                        signaldiff = AlignUtil.getFlowSignalFromBaseCall(getBC(qscore));
                        signaldiff = (qscore < signaldiff) ? (signaldiff - qscore) : (qscore - signaldiff);
                    }
        ...
        if (endLocal) {
            for (j = 1; j <= flowTseq.getLength(); j++) { // target
                ...
                if (0 < tscore && this.score <= dp[flowQseq.getLength()][j].delScore) {
        ...
        while (0 < i) { // qseq flows left
            ...
            if (FROM_MATCH == cType) {
                ...
                if (dp[i][j].insFrom == FROM_MATCHEMPTY || dp[i][j].insFrom == FROM_INSEMPTY) {
                    ...
                    i--;
                } else if (dp[i][j].insFrom == FROM_SKIPPEDMATCH || dp[i][j].insFrom == FROM_SKIPPEDINS) {
                    k = (i - 1) % qseqFlowOrder.getLength();
                    iFrom = ((i < qseqFlowOrder.jumpRev[k]) ? 0 : (i - qseqFlowOrder.jumpRev[k]));
                    while (iFrom < i) {
                        k = (i - 1) % qseqFlowOrder.getLength();
            ...
            switch (nextCType) {
                ...
                case FROM_SKIPPEDMATCH:
                    cType = FROM_MATCH;
                    break;
                case FROM_INSEMPTY:
                case FROM_SKIPPEDINS:
                    cType = FROM_INS;
                    break;
                default:
                    throw new Exception("bug encountered");
     ...
     * implemented yet.]
     */
    public IonogramAlignment(IonogramAlignment modelAlign, boolean fullyClone) throws Exception {
        if (fullyClone) {
